...
- единая первая задача бесплатна: пользователь может отправить её текстом или фото;
- после общей бесплатной попытки фоторазбор стоит 100 Telegram Stars или 5 оплаченных
  разборов;
...
- после ответа кнопка объяснения для защиты уточняет текущий разбор без списания попытки.
...
Если на фото тест, обязательно перепиши каждый номер и все варианты ответа. Никогда не
спрашивай, какие номера нужно решить: на этом этапе твоя единственная задача - извлечь
всё видимое содержание фотографии.
...
Ты помогаешь студенту устно защитить уже полученное решение. На основе условия и готового
разбора составь короткое объяснение на 30-60 секунд от первого лица. Укажи главную идею,
2-4 ключевых шага и способ быстро проверить результат. Не решай новую задачу, не меняй
исходные данные и не придумывай отсутствующие факты. Если в исходном ответе есть
неопределённость, честно обозначь её. Пиши по-русски, естественно и без рекламного текста.
Никогда не используй символы Unicode U+2014 и U+2013 - только обычный дефис.
Используй простой Markdown без таблиц.
""".strip()
...
class AIService:
    ...
    def answer_photo_session(
        ...
        normalized_request = request.casefold().strip()
        has_selection = bool(re.search(
            r"\b(?:\d+|все|всё|весь|перв|втор|трет|четверт|пят|шест|седьм|восьм|девят|десят)",
            normalized_request,
        ))
        effective_request = request.strip()
        if not has_selection and not previous_request.strip():
            effective_request = (
                "Реши все задания, распознанные на фотографии. "
                f"Исходная просьба пользователя: {effective_request}"
            )
        previous_context = (
        ...
        prompt = (
            ...
            f"{effective_request}\n\n"
            f"{previous_context}"
            "Сохранённый текст выше является условиями задач из фотографии. Используй его "
            "как основной источник и не утверждай, что условия не приложены или не указаны. "
            "Если пользователь просит все задания или диапазон номеров, реши весь указанный "
            "набор. В остальных случаях ответь только на выбранные номера. Проси переснять "
            "фото только тогда, когда нужное условие прямо помечено как [неразборчиво]. "
            "В разделе "
    ...
    def defense_explanation(self, task_context: str, answer_text: str) -> AIAnswer:
        """Create a short defense script from an answer without solving a new task."""
        prompt = (
            "Условие или текущий запрос:\n"
            f"{task_context.strip()}\n\n"
            "Уже выданный разбор:\n"
            f"{answer_text.strip()}\n\n"
            "Подготовь объяснение этого решения для устной защиты."
        )
        return self._complete_answer([{
            "role": "user",
            "content": [{"type": "input_text", "text": prompt}],
        }], min_output_tokens=600, instructions=DEFENSE_INSTRUCTIONS)
